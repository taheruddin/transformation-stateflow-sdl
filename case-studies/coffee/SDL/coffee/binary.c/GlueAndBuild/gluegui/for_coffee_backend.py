...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_for_coffee
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_for_coffee
    tcId = i_gui_RI_for_coffee
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global for_coffee_via_shared_lib
    shared_lib = True
    for_coffee_via_shared_lib = dll.gui_for_coffee
...
def send_for_coffee_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_for_coffee(tc)
            except IOError:
                raise
    elif udp:
