...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_for_water
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_for_water
    tcId = i_gui_RI_for_water
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global for_water_via_shared_lib
    shared_lib = True
    for_water_via_shared_lib = dll.gui_for_water
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_for_water(tc)
            except IOError:
                raise
    elif udp:
