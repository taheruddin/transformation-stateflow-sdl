...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_get_init_water
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_get_init_water
    tcId = i_gui_RI_get_init_water
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global get_init_water_via_shared_lib
    shared_lib = True
    get_init_water_via_shared_lib = dll.gui_get_init_water
...
def send_get_init_water_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_get_init_water(tc)
            except IOError:
                raise
    elif udp:
