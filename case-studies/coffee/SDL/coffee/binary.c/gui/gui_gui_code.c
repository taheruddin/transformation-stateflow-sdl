...
void gui_PI_gui_polling_gui()
{
   ...
   while (!retrieve_message_from_queue(gui_RI_queue_id, gui_RI_max_msg_size, msgcontent, (int *)&message_recieved_type))
   {
      ...
      {
       case i_for_ticking : INVOKE_RI_for_ticking (msgcontent);
                 break; 
       case i_for_coffee : INVOKE_RI_for_coffee (msgcontent);
                 break; 
       case i_for_water_tmpr : INVOKE_RI_for_water_tmpr (msgcontent);
                 break; 
       case i_for_water : INVOKE_RI_for_water (msgcontent);
                 break; 
       case i_get_init_water : INVOKE_RI_get_init_water (msgcontent);
                 break; 
       case i_get_init_tmpr : INVOKE_RI_get_init_tmpr (msgcontent);
                 break; 
       default : break;
      }
   }
...
int compute_max_queue_size_element_for_RI(void)
{
   ...
   if( sizeof(T_for_ticking_message) > MAX_SIZE )
      MAX_SIZE = sizeof(T_for_ticking_message);

   if( sizeof(T_for_coffee_message) > MAX_SIZE )
      MAX_SIZE = sizeof(T_for_coffee_message);

   if( sizeof(T_for_water_tmpr_message) > MAX_SIZE )
      MAX_SIZE = sizeof(T_for_water_tmpr_message);

   if( sizeof(T_for_water_message) > MAX_SIZE )
      MAX_SIZE = sizeof(T_for_water_message);

   if( sizeof(T_get_init_water_message) > MAX_SIZE )
      MAX_SIZE = sizeof(T_get_init_water_message);

   if( sizeof(T_get_init_tmpr_message) > MAX_SIZE )
      MAX_SIZE = sizeof(T_get_init_tmpr_message);
...
void gui_startup(void)
{
   ...
   fscanf(f, "%d", &msgsize_max);
    ...
    if (sizeof(T_get_init_tmpr_message) > msgsize_max) {
        printf("[ERROR] The GUI is passing a message (get_init_tmpr) which parameter size (%d) exceeds your system limit (%d).\nYou can extend this limit by running: \n    echo NUMBER | sudo tee /proc/sys/fs/mqueue/msgsize_max  # ...with NUMBER > %d\nYou can also make it permanent (check TASTE wiki)\n\n", sizeof(T_get_init_tmpr_message), msgsize_max, sizeof(T_get_init_tmpr_message));
        exit(1);
    }

    if (sizeof(T_get_init_water_message) > msgsize_max) {
        printf("[ERROR] The GUI is passing a message (get_init_water) which parameter size (%d) exceeds your system limit (%d).\nYou can extend this limit by running: \n    echo NUMBER | sudo tee /proc/sys/fs/mqueue/msgsize_max  # ...with NUMBER > %d\nYou can also make it permanent (check TASTE wiki)\n\n", sizeof(T_get_init_water_message), msgsize_max, sizeof(T_get_init_water_message));
        exit(1);
    }

    if (sizeof(T_for_water_message) > msgsize_max) {
        printf("[ERROR] The GUI is passing a message (for_water) which parameter size (%d) exceeds your system limit (%d).\nYou can extend this limit by running: \n    echo NUMBER | sudo tee /proc/sys/fs/mqueue/msgsize_max  # ...with NUMBER > %d\nYou can also make it permanent (check TASTE wiki)\n\n", sizeof(T_for_water_message), msgsize_max, sizeof(T_for_water_message));
        exit(1);
    }

    if (sizeof(T_for_water_tmpr_message) > msgsize_max) {
        printf("[ERROR] The GUI is passing a message (for_water_tmpr) which parameter size (%d) exceeds your system limit (%d).\nYou can extend this limit by running: \n    echo NUMBER | sudo tee /proc/sys/fs/mqueue/msgsize_max  # ...with NUMBER > %d\nYou can also make it permanent (check TASTE wiki)\n\n", sizeof(T_for_water_tmpr_message), msgsize_max, sizeof(T_for_water_tmpr_message));
        exit(1);
    }

    if (sizeof(T_for_coffee_message) > msgsize_max) {
        printf("[ERROR] The GUI is passing a message (for_coffee) which parameter size (%d) exceeds your system limit (%d).\nYou can extend this limit by running: \n    echo NUMBER | sudo tee /proc/sys/fs/mqueue/msgsize_max  # ...with NUMBER > %d\nYou can also make it permanent (check TASTE wiki)\n\n", sizeof(T_for_coffee_message), msgsize_max, sizeof(T_for_coffee_message));
        exit(1);
    }

    if (sizeof(T_for_ticking_message) > msgsize_max) {
        printf("[ERROR] The GUI is passing a message (for_ticking) which parameter size (%d) exceeds your system limit (%d).\nYou can extend this limit by running: \n    echo NUMBER | sudo tee /proc/sys/fs/mqueue/msgsize_max  # ...with NUMBER > %d\nYou can also make it permanent (check TASTE wiki)\n\n", sizeof(T_for_ticking_message), msgsize_max, sizeof(T_for_ticking_message));
        exit(1);
    }
...

   {
    ...
    if (NULL != gui_queue_name) {
       ...
       create_exchange_queue(gui_queue_name, 30, gui_RI_max_msg_size, &gui_RI_queue_id);
