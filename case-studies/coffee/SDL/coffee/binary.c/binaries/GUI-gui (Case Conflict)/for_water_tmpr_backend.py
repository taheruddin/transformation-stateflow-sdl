...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_for_water_tmpr
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_for_water_tmpr
    tcId = i_gui_RI_for_water_tmpr
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global for_water_tmpr_via_shared_lib
    shared_lib = True
    for_water_tmpr_via_shared_lib = dll.gui_for_water_tmpr
...
def send_for_water_tmpr_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_for_water_tmpr(tc)
            except IOError:
                raise
    elif udp:
