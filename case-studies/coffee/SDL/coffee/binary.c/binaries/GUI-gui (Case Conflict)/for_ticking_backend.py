...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_for_ticking
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_for_ticking
    tcId = i_gui_RI_for_ticking
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global for_ticking_via_shared_lib
    shared_lib = True
    for_ticking_via_shared_lib = dll.gui_for_ticking
...
def send_for_ticking_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_for_ticking(tc)
            except IOError:
                raise
    elif udp:
