...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_get_init_tmpr
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_get_init_tmpr
    tcId = i_gui_RI_get_init_tmpr
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global get_init_tmpr_via_shared_lib
    shared_lib = True
    get_init_tmpr_via_shared_lib = dll.gui_get_init_tmpr
...
def send_get_init_tmpr_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_get_init_tmpr(tc)
            except IOError:
                raise
    elif udp:
