...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_transaction_succeseeded
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_transaction_succeseeded
    tcId = i_gui_RI_transaction_succeseeded
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global transaction_succeseeded_via_shared_lib
    shared_lib = True
    transaction_succeseeded_via_shared_lib = dll.gui_transaction_succeseeded
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.MyString_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def send_transaction_succeseeded_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_transaction_succeseeded(tc)
            except IOError:
                raise
    elif udp:
        ...
        # transaction_succeseeded_via_shared_lib(ptr)
        # The call to the dll will be done by the SDL handler that manages
        # the Undo stack and the the overal system state
        send_via_dll.dll.emit("transaction_succeseeded",
                              transaction_succeseeded_via_shared_lib,
                              tc)


def fromPysideToASN1(_):
    print('DEPRECATED call to "fromPysideToASN1"')
    # Create a native ASN.1 variable of type MyString
    # transaction_succeseeded = ASN1.MyString()
    return None
