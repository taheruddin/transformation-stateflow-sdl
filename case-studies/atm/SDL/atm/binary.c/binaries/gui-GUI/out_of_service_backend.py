...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_out_of_service
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_out_of_service
    tcId = i_gui_RI_out_of_service
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global out_of_service_via_shared_lib
    shared_lib = True
    out_of_service_via_shared_lib = dll.gui_out_of_service
...
def send_out_of_service_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_out_of_service(tc)
            except IOError:
                raise
    elif udp:
