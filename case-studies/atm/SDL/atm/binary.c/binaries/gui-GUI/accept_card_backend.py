...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_accept_card
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_accept_card
    tcId = i_gui_RI_accept_card
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global accept_card_via_shared_lib
    shared_lib = True
    accept_card_via_shared_lib = dll.gui_accept_card
...
def send_accept_card_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_accept_card(tc)
            except IOError:
                raise
    elif udp:
