...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_reject_transaction
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_reject_transaction
    tcId = i_gui_RI_reject_transaction
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global reject_transaction_via_shared_lib
    shared_lib = True
    reject_transaction_via_shared_lib = dll.gui_reject_transaction
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_reject_transaction(tc)
            except IOError:
                raise
    elif udp:
        ...
        # reject_transaction_via_shared_lib(ptr)
        # The call to the dll will be done by the SDL handler that manages
        # the Undo stack and the the overal system state
        send_via_dll.dll.emit("reject_transaction",
                              reject_transaction_via_shared_lib,
                              tc)
...
def fromPysideToASN1(_):
    ...
    # reject_transaction = ASN1.MyString()
    return None
