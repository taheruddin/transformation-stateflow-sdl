...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_digit
    tcId = i_gui_RI_digit
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global digit_via_shared_lib
    shared_lib = True
    digit_via_shared_lib = dll.gui_digit
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.Num_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.Num_REQUIRED_BYTES_FOR_ENCODING)
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_digit(tc)
            except IOError:
                raise
    elif udp:
