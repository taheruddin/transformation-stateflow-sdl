...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_other_amount
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_other_amount
    tcId = i_gui_RI_other_amount
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global other_amount_via_shared_lib
    shared_lib = True
    other_amount_via_shared_lib = dll.gui_other_amount
...
def send_other_amount_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_other_amount(tc)
            except IOError:
                raise
    elif udp:
