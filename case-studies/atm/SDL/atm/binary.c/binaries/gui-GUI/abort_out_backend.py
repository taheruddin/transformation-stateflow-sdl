...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_abort_out
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_abort_out
    tcId = i_gui_RI_abort_out
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global abort_out_via_shared_lib
    shared_lib = True
    abort_out_via_shared_lib = dll.gui_abort_out
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_abort_out(tc)
            except IOError:
                raise
    elif udp:
