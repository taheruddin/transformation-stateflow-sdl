-- Code automatically generated by asn1scc tool
WITH adaasn1rtl;
USE adaasn1rtl;
use type adaasn1rtl.OctetBuffer;
use type adaasn1rtl.BitArray;
use type adaasn1rtl.Asn1UInt;
use type adaasn1rtl.Asn1Int;
use type adaasn1rtl.BIT;
WITH System;
WITH TASTE_BasicTypes;
--# inherit adaasn1rtl,TASTE_BasicTypes;


PACKAGE TASTE_Dataview
IS


-----------------------------------------------------------asn1SccMyReal-------------------------------------------------------------
SUBTYPE asn1SccMyReal IS adaasn1rtl.Asn1Real;

asn1SccMyReal_REQUIRED_BYTES_FOR_ENCODING:CONSTANT INTEGER := 13;
asn1SccMyReal_REQUIRED_BITS_FOR_ENCODING:CONSTANT INTEGER := 104;
asn1SccMyReal_REQUIRED_BYTES_FOR_ACN_ENCODING:CONSTANT INTEGER := 13;
asn1SccMyReal_REQUIRED_BITS_FOR_ACN_ENCODING:CONSTANT INTEGER := 104;
asn1SccMyReal_REQUIRED_BYTES_FOR_XER_ENCODING:CONSTANT INTEGER :=  0;

FUNCTION asn1SccMyReal_Init return asn1SccMyReal;
---# derives val from;
FUNCTION asn1SccMyReal_IsConstraintValid(val : in asn1SccMyReal) return adaasn1rtl.ASN1_RESULT;
---# derives result from val;
FUNCTION asn1SccMyReal_Equal(val1, val2: in asn1SccMyReal) return Boolean;



ERR_asn1SccMyReal:CONSTANT INTEGER := 1005; -- (0.00000000000000000000E+000 .. 1.00000000000000000000E+003)

-----------------------------------------------------------asn1SccMyString-------------------------------------------------------------
SUBTYPE asn1SccMyString_index is integer range 1..255;
SUBTYPE asn1SccMyString_array IS adaasn1rtl.OctetBuffer(asn1SccMyString_index);
SUBTYPE asn1SccMyString_length_index is integer range 0..255;
TYPE asn1SccMyString IS  RECORD
    Length : asn1SccMyString_length_index;
    Data  : asn1SccMyString_array;
END RECORD;

asn1SccMyString_REQUIRED_BYTES_FOR_ENCODING:CONSTANT INTEGER := 256;
asn1SccMyString_REQUIRED_BITS_FOR_ENCODING:CONSTANT INTEGER := 2048;
asn1SccMyString_REQUIRED_BYTES_FOR_ACN_ENCODING:CONSTANT INTEGER := 256;
asn1SccMyString_REQUIRED_BITS_FOR_ACN_ENCODING:CONSTANT INTEGER := 2048;
asn1SccMyString_REQUIRED_BYTES_FOR_XER_ENCODING:CONSTANT INTEGER :=  0;

FUNCTION asn1SccMyString_Init return asn1SccMyString;
---# derives val from;
FUNCTION asn1SccMyString_IsConstraintValid(val : in asn1SccMyString) return adaasn1rtl.ASN1_RESULT;
---# derives result from val;
FUNCTION asn1SccMyString_Equal(val1, val2: in asn1SccMyString) return Boolean;

FUNCTION "=" (val1, val2: in asn1SccMyString) return Boolean renames asn1SccMyString_Equal;

ERR_asn1SccMyString:CONSTANT INTEGER := 1006; -- (SIZE(0 .. 255))

-----------------------------------------------------------asn1SccNum-------------------------------------------------------------
SUBTYPE asn1SccNum IS adaasn1rtl.Asn1Int range 0..9;

asn1SccNum_REQUIRED_BYTES_FOR_ENCODING:CONSTANT INTEGER := 1;
asn1SccNum_REQUIRED_BITS_FOR_ENCODING:CONSTANT INTEGER := 4;
asn1SccNum_REQUIRED_BYTES_FOR_ACN_ENCODING:CONSTANT INTEGER := 1;
asn1SccNum_REQUIRED_BITS_FOR_ACN_ENCODING:CONSTANT INTEGER := 4;
asn1SccNum_REQUIRED_BYTES_FOR_XER_ENCODING:CONSTANT INTEGER :=  0;

FUNCTION asn1SccNum_Init return asn1SccNum;
---# derives val from;
FUNCTION asn1SccNum_IsConstraintValid(val : in asn1SccNum) return adaasn1rtl.ASN1_RESULT;
---# derives result from val;
FUNCTION asn1SccNum_Equal(val1, val2: in asn1SccNum) return Boolean;



ERR_asn1SccNum:CONSTANT INTEGER := 1007; -- (0 .. 9)

-----------------------------------------------------------asn1SccAmount-------------------------------------------------------------
SUBTYPE asn1SccAmount IS adaasn1rtl.Asn1Int range 0..999999;

asn1SccAmount_REQUIRED_BYTES_FOR_ENCODING:CONSTANT INTEGER := 3;
asn1SccAmount_REQUIRED_BITS_FOR_ENCODING:CONSTANT INTEGER := 20;
asn1SccAmount_REQUIRED_BYTES_FOR_ACN_ENCODING:CONSTANT INTEGER := 3;
asn1SccAmount_REQUIRED_BITS_FOR_ACN_ENCODING:CONSTANT INTEGER := 20;
asn1SccAmount_REQUIRED_BYTES_FOR_XER_ENCODING:CONSTANT INTEGER :=  0;

FUNCTION asn1SccAmount_Init return asn1SccAmount;
---# derives val from;
FUNCTION asn1SccAmount_IsConstraintValid(val : in asn1SccAmount) return adaasn1rtl.ASN1_RESULT;
---# derives result from val;
FUNCTION asn1SccAmount_Equal(val1, val2: in asn1SccAmount) return Boolean;



ERR_asn1SccAmount:CONSTANT INTEGER := 1008; -- (0 .. 999999)


 

private
   --# hide TASTE_Dataview;


END TASTE_Dataview;