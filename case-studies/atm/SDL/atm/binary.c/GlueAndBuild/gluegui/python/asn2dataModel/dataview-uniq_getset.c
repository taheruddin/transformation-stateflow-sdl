...
{
    memcpy(dest, src, sizeof(Amount));
}

byte* MovePtrBySizeOf_Amount(byte *pData)
{
    return pData + sizeof(Amount);
}

byte* CreateInstanceOf_Amount() {
    Amount *p = (Amount*)malloc(sizeof(Amount));
    Amount_Initialize(p);
    return (byte*)p;
}
...
{
    return pData + sizeof(Num);
}

byte* CreateInstanceOf_Num() {
