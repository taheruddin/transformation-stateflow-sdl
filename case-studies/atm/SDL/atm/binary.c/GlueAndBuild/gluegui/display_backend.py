...
def setMsgQ():
    global tmId
    tmId = PythonController.i_display


def setUDP():
    global tmId
    from _InterfaceEnum import i_gui_PI_display
    tmId = i_gui_PI_display


def display(tm_ptr, size):
    ...
    while True:
        ...
        if msgId == tmId:
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received display with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected display (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print 'Received other (%s), but still waiting for display' % str(msgId)
            Q.task_done()
