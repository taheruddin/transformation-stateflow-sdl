...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_amount
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_amount
    tcId = i_gui_RI_amount
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global amount_via_shared_lib
    shared_lib = True
    amount_via_shared_lib = dll.gui_amount
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_amount(tc)
            except IOError:
                raise
    elif udp:
