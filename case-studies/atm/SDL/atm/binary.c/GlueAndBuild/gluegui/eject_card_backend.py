...
def setMsgQ():
    global tmId
    tmId = PythonController.i_eject_card


def setUDP():
    global tmId
    from _InterfaceEnum import i_gui_PI_eject_card
    tmId = i_gui_PI_eject_card


def eject_card(tm_ptr, size):
...
def expect(Q, VNvalue, ignoreOther=False, timeout=None):
    ...
    while True:
        ...
        if msgId == tmId:
            #expectedValue = '{ eject-card ' + VNvalue + ' }'
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received eject-card with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected eject-card (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print 'Received other (%s), but still waiting for eject-card' % str(msgId)
            Q.task_done()
