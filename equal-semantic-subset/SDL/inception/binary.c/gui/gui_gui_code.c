...
void gui_PI_gui_polling_gui()
{
   ...
   while (!retrieve_message_from_queue(gui_RI_queue_id, gui_RI_max_msg_size, msgcontent, (int *)&message_recieved_type))
   {
      ...
      {
       case i_inmsg : INVOKE_RI_inmsg (msgcontent);
                 break; 
       default : break;
      }
   }
...
{
   //Create a variable of type T_outmsg__data to be contained in the queue
   T_outmsg__data outmsg__data;

   outmsg__data.msg = *msg;

   write_message_to_queue(gui_PI_queue_id, sizeof(T_outmsg__data), (void*)&outmsg__data, i_outmsg);

   write_message_to_queue(gui_PI_Python_queue_id, sizeof(T_outmsg__data), (void*)&outmsg__data, i_outmsg);
...
int compute_max_queue_size_element_for_RI(void)
{
   ...
   if( sizeof(T_inmsg_message) > MAX_SIZE )
      MAX_SIZE = sizeof(T_inmsg_message);
...
int compute_max_queue_size_element_for_PI(void)
{
   ...
   if( sizeof(T_outmsg_message) > MAX_SIZE )
      MAX_SIZE = sizeof(T_outmsg_message);
...
void gui_startup(void)
{
   ...
   fscanf(f, "%d", &msgsize_max);
    ...
    if (sizeof(T_outmsg_message) > msgsize_max) {
        printf("[ERROR] The GUI is passing a message (outmsg) which parameter size (%d) exceeds your system limit (%d).\nYou can extend this limit by running: \n    echo NUMBER | sudo tee /proc/sys/fs/mqueue/msgsize_max  # ...with NUMBER > %d\nYou can also make it permanent (check TASTE wiki)\n\n", sizeof(T_outmsg_message), msgsize_max, sizeof(T_outmsg_message));
        exit(1);
    }

    if (sizeof(T_inmsg_message) > msgsize_max) {
        printf("[ERROR] The GUI is passing a message (inmsg) which parameter size (%d) exceeds your system limit (%d).\nYou can extend this limit by running: \n    echo NUMBER | sudo tee /proc/sys/fs/mqueue/msgsize_max  # ...with NUMBER > %d\nYou can also make it permanent (check TASTE wiki)\n\n", sizeof(T_inmsg_message), msgsize_max, sizeof(T_inmsg_message));
        exit(1);
    }
