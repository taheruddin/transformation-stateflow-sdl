...
def setMsgQ():
    global tmId
    tmId = PythonController.i_outmsg


def setUDP():
    global tmId
    from _InterfaceEnum import i_gui_PI_outmsg
    tmId = i_gui_PI_outmsg


def outmsg(tm_ptr, size):
    ...
    while True:
        ...
        if msgId == tmId:
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received outmsg with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected outmsg (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print 'Received other (%s), but still waiting for outmsg' % str(msgId)
            Q.task_done()
