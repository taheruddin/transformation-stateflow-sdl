...
T_gui_PI_list ii_outmsg = i_outmsg;
T_gui_RI_list ii_inmsg = i_inmsg;
typedef struct {
    int tc_id;
    MyString whatever;
} inmsg_TCDATA;

int SendTC_inmsg(void *p_whatever)
{
    ...
    inmsg_TCDATA data;
    data.tc_id = (int) i_inmsg;
    data.whatever = * (MyString *) p_whatever;
    if (((mqd_t)-1) != q) {
        write_message_to_queue(q, sizeof(inmsg_TCDATA)-4, &data.whatever, data.tc_id);
    } else {
        return -1;
    }
    return 0;
