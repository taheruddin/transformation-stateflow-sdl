...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_inmsg
    tcId = i_gui_RI_inmsg
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global inmsg_via_shared_lib
    shared_lib = True
    inmsg_via_shared_lib = dll.gui_inmsg
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_inmsg(tc)
            except IOError:
                raise
    elif udp:
