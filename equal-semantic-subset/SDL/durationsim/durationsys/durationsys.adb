    ...
    procedure runTransition(Id: Integer) is
        ...
        begin
            while (trId /= -1) loop
                case trId is
                    when 0 =>
                        -- NEXT_STATE WAIT (11,18) at 604, 282
                        trId := -1;
                        ctxt.state := WAIT;
                    ...
                    when 1 =>
                        ...
                        if (ctxt.someval) = 3 then
                            -- NEXT_STATE TICKING (21,30) at 802, 555
                            trId := -1;
                            ctxt.state := TICKING;
                            goto next_transition;
                            -- ANSWER ELSE (None,None)
                        else
                            -- reset_timer(sdltmr) (25,25)
                            RESET_sdltmr;
                            -- outputmsg('WAIT') (27,27)
                            tmp6 := (Data => (87, 65, 73, 84, others => 0), Length => 4);
                            RIÜoutputmsg(tmp6'Access);
                            -- NEXT_STATE WAIT (29,30) at 923, 665
                            trId := -1;
                            ctxt.state := WAIT;
                            goto next_transition;
                        end if;
                    when 2 =>
                        -- outputmsg('READY') (34,19)
                        tmp8 := (Data => (82, 69, 65, 68, 89, others => 0), Length => 5);
                        RIÜoutputmsg(tmp8'Access);
                        -- NEXT_STATE READY (36,22) at 1079, 497
                        trId := -1;
                        ctxt.state := READY;
                        goto next_transition;
                    when 3 =>
                        ...
                        if (ctxt.someval) = 3 then
                            ...
                            -- NEXT_STATE TICKING (60,30) at 536, 610
                            trId := -1;
                            ctxt.state := TICKING;
                            goto next_transition;
                            -- ANSWER ELSE (None,None)
                            ...
                            -- NEXT_STATE WAIT (66,30) at 691, 560
                            trId := -1;
                            ctxt.state := WAIT;
                            goto next_transition;
                        end if;
