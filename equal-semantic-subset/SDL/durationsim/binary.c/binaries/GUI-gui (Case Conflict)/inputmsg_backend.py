...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_inputmsg
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_inputmsg
    tcId = i_gui_RI_inputmsg
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global inputmsg_via_shared_lib
    shared_lib = True
    inputmsg_via_shared_lib = dll.gui_inputmsg
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_inputmsg(tc)
            except IOError:
                raise
    elif udp:
