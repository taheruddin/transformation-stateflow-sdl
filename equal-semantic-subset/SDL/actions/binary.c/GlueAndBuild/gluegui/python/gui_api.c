...
int SendTC_push(void *p_w)
{
    ...
    push_TCDATA data;
    data.tc_id = (int) i_push;
    data.w = * (MyString *) p_w;
    if (((mqd_t)-1) != q) {
        write_message_to_queue(q, sizeof(push_TCDATA)-4, &data.w, data.tc_id);
    } else {
        return -1;
    }
    return 0;
}
T_gui_RI_list ii_tap = i_tap;
typedef struct {
    int tc_id;
    MyString w;
} tap_TCDATA;
...
int SendTC_tap(void *p_w)
{
    ...
    tap_TCDATA data;
    data.tc_id = (int) i_tap;
    data.w = * (MyString *) p_w;
    if (((mqd_t)-1) != q) {
        write_message_to_queue(q, sizeof(tap_TCDATA)-4, &data.w, data.tc_id);
    } else {
        return -1;
    }
    return 0;
