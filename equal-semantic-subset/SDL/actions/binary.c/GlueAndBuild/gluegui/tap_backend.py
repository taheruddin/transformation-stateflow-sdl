...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_tap
    tcId = i_gui_RI_tap
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global tap_via_shared_lib
    shared_lib = True
    tap_via_shared_lib = dll.gui_tap
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_tap(tc)
            except IOError:
                raise
    elif udp:
