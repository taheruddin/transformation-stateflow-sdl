...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_push
    tcId = i_gui_RI_push
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global push_via_shared_lib
    shared_lib = True
    push_via_shared_lib = dll.gui_push
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_push(tc)
            except IOError:
                raise
    elif udp:
