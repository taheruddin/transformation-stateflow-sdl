...
void gui_PI_gui_polling_gui()
{
   ...
   while (!retrieve_message_from_queue(gui_RI_queue_id, gui_RI_max_msg_size, msgcontent, (int *)&message_recieved_type))
   {
      ...
      {
       case i_tap : INVOKE_RI_tap (msgcontent);
                 break; 
       case i_push : INVOKE_RI_push (msgcontent);
                 break; 
       default : break;
      }
   }
...
int compute_max_queue_size_element_for_RI(void)
{
   ...
   if( sizeof(T_tap_message) > MAX_SIZE )
      MAX_SIZE = sizeof(T_tap_message);

   if( sizeof(T_push_message) > MAX_SIZE )
      MAX_SIZE = sizeof(T_push_message);
...
void gui_startup(void)
{
   ...
   fscanf(f, "%d", &msgsize_max);
    ...
    if (sizeof(T_push_message) > msgsize_max) {
        printf("[ERROR] The GUI is passing a message (push) which parameter size (%d) exceeds your system limit (%d).\nYou can extend this limit by running: \n    echo NUMBER | sudo tee /proc/sys/fs/mqueue/msgsize_max  # ...with NUMBER > %d\nYou can also make it permanent (check TASTE wiki)\n\n", sizeof(T_push_message), msgsize_max, sizeof(T_push_message));
        exit(1);
    }

    if (sizeof(T_tap_message) > msgsize_max) {
        printf("[ERROR] The GUI is passing a message (tap) which parameter size (%d) exceeds your system limit (%d).\nYou can extend this limit by running: \n    echo NUMBER | sudo tee /proc/sys/fs/mqueue/msgsize_max  # ...with NUMBER > %d\nYou can also make it permanent (check TASTE wiki)\n\n", sizeof(T_tap_message), msgsize_max, sizeof(T_tap_message));
        exit(1);
    }
