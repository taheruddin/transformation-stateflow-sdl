...
# Generate code for OpenGEODE function guard_3
cd "$SKELS"/guard_3 && opengeode --toAda guard_3.pr system_structure.pr && cd $OLDPWD

cd "$SKELS" && rm -f guard_3.zip && zip guard_3 guard_3/* && cd $OLDPWD
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	-o "$OUTPUTDIR" \
	--subAda guard_3:"$SKELS"/guard_3.zip \
