...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_stop_recording
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_stop_recording
    tcId = i_gui_RI_stop_recording
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global stop_recording_via_shared_lib
    shared_lib = True
    stop_recording_via_shared_lib = dll.gui_stop_recording
...
def send_stop_recording_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_stop_recording(tc)
            except IOError:
                raise
    elif udp:
