...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_switch_on
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_switch_on
    tcId = i_gui_RI_switch_on
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global switch_on_via_shared_lib
    shared_lib = True
    switch_on_via_shared_lib = dll.gui_switch_on
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_switch_on(tc)
            except IOError:
                raise
    elif udp:
