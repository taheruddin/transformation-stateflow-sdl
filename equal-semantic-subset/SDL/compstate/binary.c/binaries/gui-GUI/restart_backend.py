...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_restart
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_restart
    tcId = i_gui_RI_restart
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global restart_via_shared_lib
    shared_lib = True
    restart_via_shared_lib = dll.gui_restart
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_restart(tc)
            except IOError:
                raise
    elif udp:
