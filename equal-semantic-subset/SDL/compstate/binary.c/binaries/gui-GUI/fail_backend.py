...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_fail
    tcId = i_gui_RI_fail
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global fail_via_shared_lib
    shared_lib = True
    fail_via_shared_lib = dll.gui_fail
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_fail(tc)
            except IOError:
                raise
    elif udp:
