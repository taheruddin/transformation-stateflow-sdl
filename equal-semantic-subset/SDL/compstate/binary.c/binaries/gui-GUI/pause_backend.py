...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_pause
    tcId = i_gui_RI_pause
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global pause_via_shared_lib
    shared_lib = True
    pause_via_shared_lib = dll.gui_pause
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_pause(tc)
            except IOError:
                raise
    elif udp:
