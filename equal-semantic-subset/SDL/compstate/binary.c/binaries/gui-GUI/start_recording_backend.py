...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_start_recording
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_start_recording
    tcId = i_gui_RI_start_recording
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global start_recording_via_shared_lib
    shared_lib = True
    start_recording_via_shared_lib = dll.gui_start_recording
...
def send_start_recording_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_start_recording(tc)
            except IOError:
                raise
    elif udp:
