...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_switch_off
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_switch_off
    tcId = i_gui_RI_switch_off
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global switch_off_via_shared_lib
    shared_lib = True
    switch_off_via_shared_lib = dll.gui_switch_off
...
def send_switch_off_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_switch_off(tc)
            except IOError:
                raise
    elif udp:
