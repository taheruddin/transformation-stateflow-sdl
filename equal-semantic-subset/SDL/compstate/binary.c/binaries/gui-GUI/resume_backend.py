...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_resume
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_resume
    tcId = i_gui_RI_resume
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global resume_via_shared_lib
    shared_lib = True
    resume_via_shared_lib = dll.gui_resume
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_resume(tc)
            except IOError:
                raise
    elif udp:
