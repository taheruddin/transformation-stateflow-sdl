...
# Generate code for OpenGEODE function compstate
cd "$SKELS"/compstate && opengeode --toAda compstate.pr system_structure.pr && cd $OLDPWD

cd "$SKELS" && rm -f compstate.zip && zip compstate compstate/* && cd $OLDPWD
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	-o "$OUTPUTDIR" \
	--subAda compstate:"$SKELS"/compstate.zip \
