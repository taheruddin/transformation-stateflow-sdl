...
# Generate code for OpenGEODE function parallel
cd "$SKELS"/parallel && opengeode --toAda parallel.pr system_structure.pr && cd $OLDPWD

cd "$SKELS" && rm -f parallel.zip && zip parallel parallel/* && cd $OLDPWD
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	-o "$OUTPUTDIR" \
	--subAda parallel:"$SKELS"/parallel.zip \
