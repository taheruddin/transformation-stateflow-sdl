...
void gui_PI_gui_polling_gui()
{
   ...
   while (!retrieve_message_from_queue(gui_RI_queue_id, gui_RI_max_msg_size, msgcontent, (int *)&message_recieved_type))
   {
      ...
      {
       case i_check : INVOKE_RI_check (msgcontent);
                 break; 
       case i_get_data : INVOKE_RI_get_data (msgcontent);
                 break; 
       default : break;
      }
   }
...
int compute_max_queue_size_element_for_RI(void)
{
   ...
   if( sizeof(T_check_message) > MAX_SIZE )
      MAX_SIZE = sizeof(T_check_message);

   if( sizeof(T_get_data_message) > MAX_SIZE )
      MAX_SIZE = sizeof(T_get_data_message);
...
void gui_startup(void)
{
   ...
   fscanf(f, "%d", &msgsize_max);
    ...
    if (sizeof(T_get_data_message) > msgsize_max) {
        printf("[ERROR] The GUI is passing a message (get_data) which parameter size (%d) exceeds your system limit (%d).\nYou can extend this limit by running: \n    echo NUMBER | sudo tee /proc/sys/fs/mqueue/msgsize_max  # ...with NUMBER > %d\nYou can also make it permanent (check TASTE wiki)\n\n", sizeof(T_get_data_message), msgsize_max, sizeof(T_get_data_message));
        exit(1);
    }

    if (sizeof(T_check_message) > msgsize_max) {
        printf("[ERROR] The GUI is passing a message (check) which parameter size (%d) exceeds your system limit (%d).\nYou can extend this limit by running: \n    echo NUMBER | sudo tee /proc/sys/fs/mqueue/msgsize_max  # ...with NUMBER > %d\nYou can also make it permanent (check TASTE wiki)\n\n", sizeof(T_check_message), msgsize_max, sizeof(T_check_message));
        exit(1);
    }
...

   {
    ...
    if (NULL != gui_queue_name) {
       ...
       create_exchange_queue(gui_queue_name, 10, gui_RI_max_msg_size, &gui_RI_queue_id);
