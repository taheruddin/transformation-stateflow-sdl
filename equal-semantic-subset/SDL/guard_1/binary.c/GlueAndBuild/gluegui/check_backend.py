...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_check
    tcId = i_gui_RI_check
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global check_via_shared_lib
    shared_lib = True
    check_via_shared_lib = dll.gui_check
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_check(tc)
            except IOError:
                raise
    elif udp:
