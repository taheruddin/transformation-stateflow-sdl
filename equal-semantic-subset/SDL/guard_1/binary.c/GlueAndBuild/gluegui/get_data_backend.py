...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_get_data
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_get_data
    tcId = i_gui_RI_get_data
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global get_data_via_shared_lib
    shared_lib = True
    get_data_via_shared_lib = dll.gui_get_data
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_get_data(tc)
            except IOError:
                raise
    elif udp:
