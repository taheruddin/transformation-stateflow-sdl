...
void guard_1_RI_out_msg(const asn1SccMyString *IN_msg)
{
#ifdef __unix__
...
#ifdef __unix__
        printf ("** Encoding error in guard_1_RI_out_msg!!\n");
#endif
    ...
    extern void vm_async_guard_1_out_msg(void *, size_t);

    vm_async_guard_1_out_msg(IN_buf_msg, size_IN_buf_msg);

}

void guard_1_RI_check_queue(asn1SccT_Boolean *OUT_res)
{
#ifdef __unix__
    static int innerMsc = -1;
    if (-1 == innerMsc)
    ...
    if (1 == innerMsc) {
        ...
        printf ("\nINNER: guard_1,chip_taste_api,check_queue,%lld\n", msc_time);
        fflush(stdout);
    }
#endif
    ...
    /* Call to VM callback function */
    extern void vm_guard_1_check_queue(void *, size_t *);

    vm_guard_1_check_queue(OUT_buf_res, &size_OUT_buf_res);
