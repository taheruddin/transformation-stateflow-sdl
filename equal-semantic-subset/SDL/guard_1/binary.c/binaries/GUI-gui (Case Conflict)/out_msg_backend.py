...
def setMsgQ():
    global tmId
    tmId = PythonController.i_out_msg


def setUDP():
    global tmId
    from _InterfaceEnum import i_gui_PI_out_msg
    tmId = i_gui_PI_out_msg


def out_msg(tm_ptr, size):
    ...
    while True:
        ...
        if msgId == tmId:
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received out-msg with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected out-msg (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print 'Received other (%s), but still waiting for out-msg' % str(msgId)
            Q.task_done()
