...
# Generate code for OpenGEODE function guard_1
cd "$SKELS"/guard_1 && opengeode --toAda guard_1.pr system_structure.pr && cd $OLDPWD

cd "$SKELS" && rm -f guard_1.zip && zip guard_1 guard_1/* && cd $OLDPWD
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	-o "$OUTPUTDIR" \
	--subAda guard_1:"$SKELS"/guard_1.zip \
