...
def Invoke_inputmsg(var_T_Boolean):
    if -1 == SendTC_inputmsg(var_T_Boolean._ptr):
...
def ProcessTM(self):
    if self.messageReceivedType == i_outputmsg:
        ...
        backup = self._pMem
        # Read the data for param outgo
        var_outgo = dataview_uniq_asn.T_Boolean()
        var_outgo.SetData(self._pMem)
        print "Parameter outgo:"
        var_outgo.PrintAll()
        print
        # self._pMem = DV.MovePtrBySizeOf_T_Boolean(self._pMem)
